...
def is_sale(this_line):
   ...
   return (re.search(r'\s\$\s', this_line) and re.search(r'[0-9]', this_line))
...
def get_sale_location(word):
    ...
    sale_location = ' '.join(word).strip(', ')
    
    if ',' in sale_location:
        sale_location = sale_location.split(',')    
    else:
        match = re.search(r'(.*?)(' + scrape_util.state + ')', sale_location, re.IGNORECASE)
        if match:
...
def get_sale(word):
    ...
    if number_word == [0, 2, 3, 5]:
        ...
        sale = {
               ...
               'consignor_state': sale_location[1].upper().strip(strip_char),
...
def main():
   ...
   for this_report in report:
       ...
       try:
           ...
           continue
       line = []
       for this_line in soup.stripped_strings:
          line.extend(this_line.splitlines())   
       #line = [this_line for this_line in soup.stripped_strings]
       line = [re.sub('\xa0+', ' ', this_line) for this_line in line]
